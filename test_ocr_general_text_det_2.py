...
def single_test(args, img_folder, model_path, save_folder):
    ...
    #args.resume = "/data_2/save_model_file/PseNet/ocr_resnet50_bs_4_ep_200_2020-08-22_20_09_49_pretrain_ic17/checkpoint34.pth.tar"
    ...
    save_flag = True
    #save_root = "/data_2/result/通用模型定位结果/20200824"
    save_root = os.path.join(save_folder, os.path.splitext(os.path.basename(model_path))[0])
    ...
    #file_path = "/data_2/data/ocr/test/piaoju"
    #file_path = "/data_1/data/general_text_det/data_labeld/20200821/ICDAR票据数据/task1_train"
    #file_path = "/data_2/data/ocr/test/xiaopiao"
    ...
    for img_name in os.listdir(file_path):
        ...
        torch.cuda.synchronize()
        start = time.time()
        det_rects = general_text_detect_1(inputImg, model, 1280) #1280 640
        torch.cuda.synchronize()
        end = time.time()
...
def batch_test(args, img_folder, model_list_path, save_folder):
    with open(model_list_path) as fr:
        for model_path in fr.readlines():
            model_path = model_path.rstrip()
            single_test(args, img_folder, model_path, save_folder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--long_size', nargs='?', type=int, default=1280,
                        help='Path to previous saved model to restart from')
    parser.add_argument('--min_kernel_area', nargs='?', type=float, default=10.0,#10.0 10.0
                        help='min kernel area')
    parser.add_argument('--min_area', nargs='?', type=float, default=10.0,#300.0 10.0
                        help='min area')
    parser.add_argument('--min_score', nargs='?', type=float, default=0.8, #0.93 0.8
                        help='min score')
    ...
    if "single_test" == flag:
        # img_folder = "/data_1/data/general_text_det/data_labeld/20200901/发票/mark_rename"
        # model_path = "/data_2/save_model_file/PseNet/ocr_resnet50_bs_4_ep_200_2020-09-02_21_00_39_pretrain_ic17/checkpoint12.pth.tar"
        # save_folder = "/data_2/result/通用模型定位结果/20200903/fapiao_36"

        img_folder = "/data_2/data/ocr/test"
        model_path = "./ckpt/general_detection.tar"
        save_folder = "/data_2/data/ocr/res"
        
        single_test(args, img_folder, model_path, save_folder)
    elif "batch_test" == flag:
        img_folder = "/data_2/data/ocr/xiaopiao/百望/mark"
        model_list_path = "/data_2/save_model_file/PseNet-xiaopiao/ocr_resnet50_bs_4_ep_200_2020-11-05_17_00_11_pretrain_ic17/model_list.txt"
        save_folder = "/data_2/result/电子小票定位结果/20201203"
        batch_test(args, img_folder, model_list_path, save_folder)
