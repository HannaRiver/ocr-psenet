...
dataset_root = '/data_2/3rd/goertek/data'
data_keys = ['warehouse', "replenishment", "material_scrap"]
dates = ['20210603', '20210628','20210702', '20210609', "20210703", "20210705", "20210726", "20210728"]
model_names = ['pse_wh_200', 'pse']
model_label_types = ['right', 'error_labeled']
dataset_folders = [i + '_' + j for i in model_names for j in model_label_types] + ['human_check']
# dataset_folders = ['human_check']
ocr_train_data_dirs = [os.path.join(dataset_root, i, j, 'data4train', k, 'train', 'img') for i in data_keys for j in dates for k in dataset_folders]
ocr_train_gt_dirs = [os.path.join(dataset_root, i, j, 'data4train', k, 'train', 'ctw') for i in data_keys for j in dates for k in dataset_folders]
...
def get_bboxes(img, gt_path):
    ...
    tags = []
    for line in lines:
        #\xef\xbb\xbf是一些utf-8编码的文件，在每行开头引入BOM来表明编码方式
...
def random_scale(img, min_size):
    ...
    #最小变小于640 则让最小边大于等于650 因为后面的随机裁切是min_size*min_size的区域
...
def random_crop(imgs, img_size):
    ...
    #np.max(imgs[1]) > 0表示有文本的时候
    if random.random() > 3.0 / 8.0 and np.max(imgs[1]) > 0:
        #找到图像包含文本区域的水平外接矩形的左上角点 np.where得到的是二维索引坐标、行、列各一个列表
        tl = np.min(np.where(imgs[1] > 0), axis = 1) - img_size #减img_size是防止基于tl扩展的img_size区域超出边界和be中减img_size对应
        tl[tl < 0] = 0
        #找到图像包含文本区域的水平外接矩形的右下角点
        br = np.max(np.where(imgs[1] > 0), axis = 1) - img_size
        br[br < 0] = 0
        br[0] = min(br[0], h - th)
        br[1] = min(br[1], w - tw)
        
        i = random.randint(tl[0], br[0]) #行坐标
        j = random.randint(tl[1], br[1]) #列坐标
    else:
...
class OCRGeneralTextDetLoader(data.Dataset):
    def __init__(self, is_transform=False, img_size=None, kernel_num=7, min_scale=0.4):
        ...
        data_dirs = ocr_train_data_dirs
        gt_dirs = ocr_train_gt_dirs
        ...
        for data_dir, gt_dir in zip(data_dirs, gt_dirs):
            if not os.path.exists(data_dir):
                continue
            print("========== Deal with the data dir: %s ==========" %(data_dir))
            img_names = util.io.ls(data_dir, '.jpg')
            img_names.extend(util.io.ls(data_dir, '.png'))
            img_names.extend(util.io.ls(data_dir, '.tif'))
            ...
            for idx, img_name in enumerate(img_names):
                ...
                elif img_name.find('.tif') != -1:
                    gt_name = img_name.split('.tif')[0] + '.txt'
                else:
    ...
    def __getitem__(self, index):
        ...
        if bboxes.shape[0] > 0:
            #bboxes * ([img.shape[1], img.shape[0]] * 14 是将归一化的14个坐标点的坐标 换算到resize后的图片上的坐标
            #bboxes.shape[0], bboxes.shape[1] / 2, 2 是将坐标[x1,y1,x2,y2] 转成[[x1, y1], [x2, y2]]
            ...
            for i in range(bboxes.shape[0]):
                ...
                if not tags[i]: #难样例不学习，将train_mask置为0
