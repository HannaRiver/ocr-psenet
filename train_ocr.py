...
def main(args):
    if args.checkpoint == '':
        args.checkpoint = "checkpoints/ocr_%s_bs_%d_ep_%d"%(args.arch, args.batch_size, args.n_epoch)
    if args.pretrain:
    ...
    data_loader = OCRLoader(is_transform=True, img_size=args.img_size, kernel_num=kernel_num, min_scale=min_scale)
