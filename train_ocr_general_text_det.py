...
from dataset import OCRGeneralTextDetLoader
...
def dice_loss(input, target, mask):
    input = torch.sigmoid(input) #前向函数中没有加sigmoid 在这加了
    ...
    b = torch.sum(input * input, 1) + 0.001 #作者源码
    c = torch.sum(target * target, 1) + 0.001
    # b = torch.sum(input, 1) + 0.001 #自己添加
    # c = torch.sum(target, 1) + 0.001
    d = (2 * a) / (b + c)
    dice_loss = torch.mean(d)
    ##########
    # print("dice_loss: {0}".format(dice_loss))
    # print("a: {0}".format(a))
    ##########
    return 1 - dice_loss
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for batch_idx, (imgs, gt_texts, gt_kernels, training_masks) in enumerate(train_loader):
        ...
        outputs = model(imgs) #模型预测结果
        ################################
        # print("!!!!!!!!!!!!!!!!!!!!!!!")
        # print("outputs: {0}".format(outputs))
        # out_tmp = outputs[outputs < 0]
        # print("out_tmp: {0}".format(out_tmp))
        # out_tmp_1 = outputs[outputs > 0]
        # print("out_tmp_1: {0}".format(out_tmp_1))
        ################################
...
def main(args):
    if args.checkpoint == '':
        ...
        # args.checkpoint = "checkpoints/ocr_%s_bs_%d_ep_%d"%(args.arch, args.batch_size, args.n_epoch) + "_" + cur_time
        #args.checkpoint = "/data_2/save_model_file/PseNet/ocr_%s_bs_%d_ep_%d"%(args.arch, args.batch_size, args.n_epoch) + "_" + cur_time
        #args.checkpoint = "checkpoints/ocr_resnet50_bs_4_ep_200_2020-04-16_10_20_23_pretrain_ic17"
        args.checkpoint = "/home/ocr/PseNet/checkpoints/ocr_resnet50_bs_4_ep_200_2020-09-07_21_02_13_pretrain_ic17"
    if args.pretrain:
    ...
    data_loader = OCRGeneralTextDetLoader(is_transform=True, img_size=args.img_size, kernel_num=kernel_num, min_scale=min_scale)
    ...
    title = 'ocr_add_ic15_res50_pretrain_ic17'
    if args.pretrain:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain', nargs='?', type=str, default='/data_2/projects/PseNet/ckpt/checkpoint200.pth.tar',    
                        help='Path to previous saved model to restart from')
    parser.add_argument('--checkpoint', default='/data_2/projects/PseNet/ckpt/wh_bs_8_ep_200_pretrain', type=str, metavar='PATH',
